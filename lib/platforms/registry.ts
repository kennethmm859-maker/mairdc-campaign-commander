import type { Platform, PublishPayload, PublishResult, SocialAdapter } from "./types";

class PendingAdapter implements SocialAdapter {
  constructor(public platform: Platform) {}

  async publish(_payload: PublishPayload, _accessToken: string): Promise<PublishResult> {
    throw new Error(`${this.platform} adapter is installed but not enabled. Complete OAuth/app approval first.`);
  }
}
